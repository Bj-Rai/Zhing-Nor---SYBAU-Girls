...
from highway_scene import HighwayScene
from question_system import QuestionManager
from question_bank import register_question_banks
from water_tap import WaterTap
...
class ArtEngine:
    ...
    def draw_new_premium_truck():
        ...
        # ---- Side mirror ----
        pygame.draw.line(surf, outline, (28, 52), (20, 46), 2)
        pygame.draw.rect(surf, (180, 200, 210), (14, 40, 8, 10), border_radius=2)
        pygame.draw.rect(surf, outline, (14, 40, 8, 10), 1, border_radius=2)

        # ---- Tail light ----
        pygame.draw.rect(surf, (200, 40, 30), (222, 82, 5, 12), border_radius=2)
        pygame.draw.rect(surf, outline, (222, 82, 5, 12), 1, border_radius=2)

        # ---- Mud flaps ----
        for wx in [64, 172]:
            pygame.draw.rect(surf, (30, 30, 32), (wx-6, 118, 12, 10), border_radius=1)

        # ---- Exhaust pipe ----
        pygame.draw.rect(surf, (140, 140, 145), (215, 96, 6, 14), border_radius=2)

        # ---- Farm-fresh signboard on the crate side ----
        try:
            font_sign = pygame.font.Font(None, 14)
            sign_bg = pygame.Rect(100, 92, 108, 14)
            pygame.draw.rect(surf, (240, 235, 220), sign_bg, border_radius=3)
            pygame.draw.rect(surf, outline, sign_bg, 1, border_radius=3)
            sign_text = font_sign.render("FARM FRESH VEGGIES", True, (60, 100, 50))
            surf.blit(sign_text, (sign_bg.x + 6, sign_bg.y + 1))
        except:
            pass
    ...
    @staticmethod
    def draw_gate_closed_tile():
        """A single fence-tile-sized gate, closed — matches TEX_FENCE_POST_ONLY sizing exactly."""
        surf = pygame.Surface((TILE_SIZE, TILE_SIZE), pygame.SRCALPHA)
        pygame.draw.ellipse(surf, (20, 35, 15, 90), (4, TILE_SIZE-8, TILE_SIZE-8, 8))
        c_main = (150, 100, 55)
        c_dark = (105, 68, 35)
        c_highlight = (210, 165, 110)
        post_w = 6
        left_px, right_px = 2, TILE_SIZE - post_w - 2
        for px in [left_px, right_px]:
            pygame.draw.rect(surf, c_dark, (px-1, 4, post_w+2, TILE_SIZE-6), border_radius=2)
            pygame.draw.rect(surf, c_main, (px, 5, post_w, TILE_SIZE-8), border_radius=2)
        gx1, gx2 = left_px+post_w, right_px
        top_y, bot_y = 10, TILE_SIZE-12
        pygame.draw.line(surf, c_dark, (gx1, top_y), (gx2, bot_y), 5)
        pygame.draw.line(surf, c_main, (gx1, top_y), (gx2, bot_y), 3)
        pygame.draw.line(surf, c_dark, (gx1, bot_y), (gx2, top_y), 5)
        pygame.draw.line(surf, c_main, (gx1, bot_y), (gx2, top_y), 3)
        pygame.draw.rect(surf, c_dark, (gx1, 16, gx2-gx1, 5), border_radius=2)
        pygame.draw.rect(surf, c_main, (gx1, 17, gx2-gx1, 3), border_radius=2)
        return surf

    @staticmethod
    def draw_gate_open_tile():
        """Same tile size, showing just the two hinge posts and an open path between."""
        surf = pygame.Surface((TILE_SIZE, TILE_SIZE), pygame.SRCALPHA)
        pygame.draw.ellipse(surf, (20, 35, 15, 90), (4, TILE_SIZE-8, TILE_SIZE-8, 8))
        c_main = (150, 100, 55)
        c_dark = (105, 68, 35)
        ...
        for px in [left_px, right_px]:
            pygame.draw.rect(surf, c_dark, (px-1, 4, post_w+2, TILE_SIZE-6), border_radius=2)
            pygame.draw.rect(surf, c_main, (px, 5, post_w, TILE_SIZE-8), border_radius=2)
        # A thin folded panel resting against the left post, showing it's swung open
        pygame.draw.rect(surf, c_dark, (left_px+post_w, 10, 4, TILE_SIZE-18))
        return surf
    ...
    def draw_fence_rail_segment(horizontal=True):
        ...
        return surf
    @staticmethod
    def draw_gate_texture(open=False):
        """A single-tile wooden farm gate with a diagonal cross-brace.
        Posts sit flush at the tile edges to match neighboring fence tiles."""
        w, h = TILE_SIZE, TILE_SIZE
        surf = pygame.Surface((w, h), pygame.SRCALPHA)
        pygame.draw.ellipse(surf, (20, 35, 15, 90), (2, h-8, w-4, 8))
        ...
        right_px = w - post_w
        for px in [left_px, right_px]:
            pygame.draw.rect(surf, c_dark, (px-1, 2, post_w+2, h-2), border_radius=3)
            pygame.draw.rect(surf, c_main, (px, 3, post_w, h-5), border_radius=3)
            pygame.draw.rect(surf, c_highlight, (px, 3, post_w-3, h-5), border_radius=3)
        ...
        if not open:
            for ry in [top_y, bot_y]:
                pygame.draw.rect(surf, c_dark, (gx1, ry, gx2-gx1, bar_h+2), border_radius=2)
                pygame.draw.rect(surf, c_main, (gx1, ry+1, gx2-gx1, bar_h), border_radius=2)
                pygame.draw.line(surf, c_highlight, (gx1+1, ry+1), (gx2-1, ry+1), 1)

            pygame.draw.line(surf, c_dark, (gx1, top_y+bar_h), (gx2, bot_y), 5)
            pygame.draw.line(surf, c_main, (gx1, top_y+bar_h), (gx2, bot_y), 3)
            pygame.draw.line(surf, c_dark, (gx1, bot_y), (gx2, top_y+bar_h), 5)
            pygame.draw.line(surf, c_main, (gx1, bot_y), (gx2, top_y+bar_h), 3)

            pygame.draw.rect(surf, (90, 90, 95), (gx2-7, h//2-3, 5, 6), border_radius=1)

        else:
            fold_x = left_px + post_w
            panel_w = 10
            for ry in [top_y, bot_y]:
                pygame.draw.rect(surf, c_dark, (fold_x, ry, panel_w, bar_h+2), border_radius=2)
                pygame.draw.rect(surf, c_main, (fold_x, ry+1, panel_w-2, bar_h), border_radius=2)
            pygame.draw.line(surf, c_dark, (fold_x, top_y+bar_h), (fold_x+panel_w, bot_y), 3)
            pygame.draw.line(surf, c_dark, (fold_x, bot_y), (fold_x+panel_w, top_y+bar_h), 3)
    ...
    def draw_improved_scarecrow():
        # 24x24 grid, nearest-neighbor x2 => crisp chunky pixel art
        ...
        # ---- post + crossbeam ----
        pygame.draw.rect(s, wood, (11, 12, 2, 12))
        pygame.draw.line(s, wood_lt, (3, 13), (20, 13), 1)
        pygame.draw.line(s, wood, (3, 14), (20, 14), 1)

        # ---- simple straw tufts at sleeve ends ----
        pygame.draw.line(s, straw, (6, 13), (3, 11), 1)
        pygame.draw.line(s, straw_dk, (5, 14), (2, 14), 1)
        pygame.draw.line(s, straw, (6, 14), (3, 16), 1)
        pygame.draw.line(s, straw, (17, 13), (20, 11), 1)
        pygame.draw.line(s, straw_dk, (18, 14), (21, 14), 1)
        pygame.draw.line(s, straw, (17, 14), (20, 16), 1)

        # ---- sleeves + shirt ----
        pygame.draw.rect(s, shirt, (6, 13, 3, 2))
        pygame.draw.rect(s, shirt, (15, 13, 3, 2))
        pygame.draw.rect(s, shirt, (9, 14, 6, 5))
        pygame.draw.line(s, shirt_dk, (9, 18), (14, 18), 1)

        # ---- pants ----
        pygame.draw.rect(s, pants, (9, 19, 2, 3))
        pygame.draw.rect(s, pants, (13, 19, 2, 3))

        # ---- neck straw ----
        pygame.draw.line(s, straw, (10, 12), (8, 13), 1)
        pygame.draw.line(s, straw, (13, 12), (15, 13), 1)

        # ---- clean burlap head ----
        pygame.draw.rect(s, burl_sh, (9, 5, 6, 1))      # soft shadow under hat
        pygame.draw.rect(s, burlap, (8, 6, 8, 4))
        pygame.draw.rect(s, burlap, (9, 10, 6, 1))
        pygame.draw.rect(s, burlap, (10, 11, 4, 1))
        pygame.draw.rect(s, burlap, (11, 12, 2, 1))

        # ---- stern but simple face ----
        pygame.draw.line(s, face, (10, 8), (11, 9), 1)   # left eye, slight inward angle
        pygame.draw.line(s, face, (13, 8), (12, 9), 1)   # right eye, slight inward angle
        pygame.draw.lines(s, face, False, [(10, 11), (11, 11), (12, 12), (13, 11)], 1)  # simple crooked mouth

        # ---- hat, gently tilted ----
        pygame.draw.rect(s, hat, (10, 0, 5, 4))          # crown
        pygame.draw.line(s, hat_dk, (10, 3), (14, 3), 1) # band
        pygame.draw.line(s, hat, (5, 5), (18, 4), 2)     # lightly tilted brim
    ...
    def create_road_texture():
        ...
        for _ in range(30):
            ...
            pygame.draw.circle(surf, (60+shade, 60+shade, 65+shade), (x, y), 1)
        pygame.draw.rect(surf, (230, 200, 60), (TILE_SIZE//2-3, 6, 6, TILE_SIZE-12), border_radius=2)
        return surf

    @staticmethod
    def draw_traffic_light(state="red"):
        surf = pygame.Surface((TILE_SIZE, TILE_SIZE*2), pygame.SRCALPHA)
        pole_x = TILE_SIZE//2 - 3
        pygame.draw.rect(surf, (50, 50, 55), (pole_x, 30, 6, TILE_SIZE*2-35), border_radius=2)
        pygame.draw.ellipse(surf, (20,20,20,90), (pole_x-8, TILE_SIZE*2-10, 22, 8))
        box = pygame.Rect(TILE_SIZE//2-14, 4, 28, 44)
        pygame.draw.rect(surf, (30, 30, 35), box, border_radius=6)
        pygame.draw.rect(surf, (10, 10, 15), box, 2, border_radius=6)
        red_on = state == "red"
        green_on = state == "green"
        pygame.draw.circle(surf, (255,60,50) if red_on else (80,20,20), (TILE_SIZE//2, 16), 8)
        pygame.draw.circle(surf, (60,220,80) if green_on else (20,70,30), (TILE_SIZE//2, 36), 8)
        if red_on:
            pygame.draw.circle(surf, (255,150,140), (TILE_SIZE//2-3, 13), 3)
        if green_on:
            pygame.draw.circle(surf, (170,255,180), (TILE_SIZE//2-3, 33), 3)
        return surf
...
TEX_GATE_CLOSED = ArtEngine.draw_gate_texture(open=False)
TEX_GATE_OPEN = ArtEngine.draw_gate_texture(open=True)
TEX_GATE_CLOSED_TILE = ArtEngine.draw_gate_closed_tile()
TEX_GATE_OPEN_TILE = ArtEngine.draw_gate_open_tile()
TEX_FENCE_POST = ArtEngine.draw_cattle_fence_post()
TEX_CHICKEN_COOP = ArtEngine.draw_chicken_coop()
TEX_ROAD = ArtEngine.create_road_texture()
TEX_LIGHT_RED = ArtEngine.draw_traffic_light("red")
TEX_LIGHT_GREEN = ArtEngine.draw_traffic_light("green")
...
# ==========================================
class TileType(Enum):
    GRASS=0; SOIL=1; CROP=2; WATER=3; PATH=4; ROAD=5
...
class TrafficLight:
    ...
    def get_texture(self):
        return TEX_LIGHT_GREEN if self.solved else TEX_LIGHT_RED
...
def generate_math_question():
    ...
    return {
        "title": "🚦 Traffic Light Checkpoint! 🚦",
        "question": f"Solve to cross: {a} {op} {b} = ?",
...
class Gate:
    ...
    def get_texture(self):
        return TEX_GATE_OPEN if self.is_open else TEX_GATE_CLOSED

    def occupies(self, x, y):
        return y == self.y and self.x <= x < self.x + self.width

    def get_tile_texture(self):
        return TEX_GATE_OPEN_TILE if self.is_open else TEX_GATE_CLOSED_TILE
...
class QuestionPopup:
    ...
    def handle_click(self, pos, game_state, inventory):
        ...
        return False
class DailyQuiz:
    def __init__(self, question_manager, questions_per_day=3):
        self.active = False
    ...
    def try_start_quiz(self, current_day):
        ...
        if self.quiz_taken_today and current_day == self.last_day:
            return False

        questions = self.question_manager.get_daily_set(
            day=current_day,
            set_size=self.questions_per_day,
            title="Daily Farmer Quiz!",
    ...
    def _load_current_question(self):
        self.current_question = self.question_queue[self.question_index]
        self.selected_option = -1
        self.show_result = False
        self.result_text = ""
        self.result_success = False
    ...
    def check_answer(self, option_index, game_state):
        ...
        if option_index == self.current_question["correct"]:
            ...
            self.result_text = (
                ...
                f"${self.current_question['reward_money']} and "
                f"{self.current_question['reward_exp']} XP!"
            )
        else:
            self.result_success = False

            correct = self.current_question["options"][
                self.current_question["correct"]
            ]

            self.result_text = (
                f"WRONG! The correct answer is: {correct}\n"
                f"Better luck tomorrow, farmer!"
            )

        # Mark this question as asked so it does not repeat until reset.
        self.question_manager.mark_asked(
            self.current_question.get("category", "daily"),
    ...
    def draw(self, surf):
        ...
        if len(self.question_queue) > 1:
            progress_text = font_small.render(
                f"Question {self.question_index + 1}/{len(self.question_queue)}",
                True,
                (200, 200, 200),
            )
            surf.blit(
                progress_text,
                (
                    panel_x + panel_w - progress_text.get_width() - 25,
                    panel_y + 25,
                ),
            )
        ...
        if self.show_result:
            ...
            for i, line in enumerate(lines):
                res_surf = font_res.render(
                    line,
                    True,
                    COLOR_WHITE if "CORRECT" in line else (255, 200, 100),
                )
                ...
                surf.blit(
                    ...
                    (
                        panel_x + 30,
                        panel_y + panel_h - 100 + i * 25,
                    ),
                )

        cont_rect = pygame.Rect(
            panel_x + panel_w // 2 - 90,
            panel_y + panel_h - 60,
            180,
            40,
        )
        ...
        if self.question_index + 1 < len(self.question_queue):
            cont_label = "Next Question ->"
        else:
            cont_label = "Continue Farming ->"

        cont_text = font_opt.render(cont_label, True, COLOR_WHITE)
        surf.blit(
            cont_text,
            (
                cont_rect.x + cont_rect.w // 2 - cont_text.get_width() // 2,
                cont_rect.y + cont_rect.h // 2 - cont_text.get_height() // 2,
            ),
        )
...
class World:
    ...
    def _setup_fenced_pasture(self):
        ...
        # Single-tile toggleable gate at the bottom-middle
        gate_x = (px1 + px2) // 2
        self.fence_tiles.discard((gate_x, py2))
        self.gates.append(Gate(gate_x, py2, width=1))
        self.collision[py2][gate_x] = True
        ...
        for x in range(run_x1, run_x2+1):
            for y in range(run_y1, run_y2+1):
                if x == run_x1 or x == run_x2 or y == run_y1 or y == run_y2:
                    ...
                    self.collision[y][x] = True
        # Single-tile toggleable gate at the bottom-middle
        gate_x = (run_x1 + run_x2) // 2
        self.fence_tiles.discard((gate_x, run_y2))
        self.gates.append(Gate(gate_x, run_y2, width=1))
        self.collision[run_y2][gate_x] = True
    ...
    def toggle_nearby_gate(self, px, py):
        for gate in self.gates:
            near = any(abs((gate.x+i) - px) <= 1 and abs(gate.y - py) <= 1 for i in range(gate.width))
            if near:
                gate.toggle()
                for i in range(gate.width):
                    self.collision[gate.y][gate.x + i] = not gate.is_open
                return gate
        return None
    ...
    def _closed_gate_occupies(self, x, y):
        return any(g.occupies(x, y) and not g.is_open for g in self.gates)
    ...
    def draw(self, surf, cam_x, cam_y, game_state, zoom):
        ...
        for fx,fy in self.fence_tiles:
            ...
            has_left = (fx-1, fy) in self.fence_tiles or self._closed_gate_occupies(fx-1, fy)
            has_right = (fx+1, fy) in self.fence_tiles or self._closed_gate_occupies(fx+1, fy)
            has_up = (fx, fy-1) in self.fence_tiles
            has_down = (fx, fy+1) in self.fence_tiles or self._closed_gate_occupies(fx, fy+1)

            # Rail toward the left neighbor
            if has_left:
        ...
        for gate in self.gates:
            sx = (gate.x*TILE_SIZE - cam_x)*zoom
            sy = (gate.y*TILE_SIZE - cam_y)*zoom
            scaled_gate = pygame.transform.scale(gate.get_tile_texture(), (scaled_tile_size, scaled_tile_size))
            surf.blit(scaled_gate, (sx, sy))
        for gate in self.gates:
            sx = (gate.x*TILE_SIZE - cam_x)*zoom
            sy = (gate.y*TILE_SIZE - cam_y)*zoom
            scaled_gate = pygame.transform.scale(gate.get_tile_texture(), (scaled_tile_size, scaled_tile_size))
...
class Player:
    ...
    def _gate_occupies(self, x, y):
        return any(g.occupies(x, y) for g in self.gates)
...
class UI:
    ...
    def draw(self, surf, game_state, inventory, player, world, quiz_active, question_active, barn_storage_active, vending_machine_active, city_market_active):
        ...
        # Bottom hints
        save_hint = self.small_font.render("F5:Save  F9:Load  T:Tutorial  Q/E:Zoom  Shift+Arrow:Run  F:Harvest Trees  G:Feed Animals  B:Barn Storage  V:Vending Machine  M:City Market", True, (200,200,200))
...
def save_game(
    ...
    if question_manager is not None:
        data["question_manager"] = question_manager.get_state()
...
def load_game(
    ...
    if question_manager is not None:
        question_manager.load_state(data.get("question_manager", {}))
    return True
...
class Game:
    def __init__(self):
        ...
        register_question_banks(self.question_manager)
        self.quiz = DailyQuiz(
        ...
        self.world.game_state = self.game_state
                # NEW: rural water tap + bucket beside the barn (tile 20,6 = open grass)
        self.water_tap = WaterTap(13, 5, TILE_SIZE)
        self.world.collision[5][13] = True
        self.animals = [
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.running = False
            elif self.game_mode == "highway" and self.highway_scene:
                self.highway_scene.handle_event(event)
                continue
                ...
                if event.key == pygame.K_t:
                ...
                elif event.key == pygame.K_e and self.water_tap.player_near(self.player.tx, self.player.ty):
                    self.water_tap.toggle()
                    ...
                    if self.game_mode == "farm" and not self.tutorial.active and not self.quiz.active and not self.current_question and not self.barn_storage.active and not self.active_vending_machine:
                        truck_found = False
                        for typ, x, y in self.world.decor:
                            if typ == "truck" and abs(x - self.player.tx) <= 2 and abs(y - self.player.ty) <= 1:
                                self.game_mode = "highway"
                                self.highway_scene = HighwayScene(SCREEN_WIDTH, SCREEN_HEIGHT, TEX_TRUCK)
                                self.highway_scene.start()
                                truck_found = True
                                break
                        if not truck_found:
                            self.show_temporary_message("🚛 Stand near the truck to drive to the city market!", (200,200,100))
                ...
                elif event.key == pygame.K_f and not self.tutorial.active and not self.quiz.active and not self.current_question and not self.barn_storage.active and not self.active_vending_machine and not self.city_market.active and self.water_tap.can_bucket(self.player.tx, self.player.ty):
                    self.water_tap.toggle_carry(self.player)
                    ...
                    save_game(self.game_state, self.world, self.inventory, self.player, self.quiz, self.barn_storage, self.question_manager)
                    self.show_temporary_message("Game Saved!", (100,200,100))
                elif event.key == pygame.K_F9:
                    if load_game(self.game_state, self.world, self.inventory, self.player, self.quiz, self.barn_storage, self.question_manager):
                        self.world.game_state = self.game_state
                        self.game_state.inventory = self.inventory
                    ...
                    if event.key == pygame.K_g:
                        ...
                        if self.world.is_inside_pasture(px, py):
                            if self.inventory.items.get("wheat", 0) >= 1:
                                if self.game_state.energy >= 5:
                                    self.inventory.remove("wheat", 1)
                                    self.inventory.add("milk", 1)
                                    self.game_state.energy = max(0, self.game_state.energy - 5)
                                    self.game_state.add_exp(4)
                                    self.show_temporary_message("🥛 Fed & milked the cow! +1 Milk, -1 Wheat, -5 energy", (100,200,100))
                                else:
                                    self.show_temporary_message("Not enough energy! Need 5 energy!", (200,100,100))
                            else:
                                self.show_temporary_message("✗ Need 1 wheat to feed the cow!", (200,100,100))
                        elif self.world.is_inside_chicken_area(px, py):
                            if self.inventory.items.get("corn", 0) >= 1:
                                if self.game_state.energy >= 5:
                                    self.inventory.remove("corn", 1)
                                    self.inventory.add("egg", 1)
                                    self.game_state.energy = max(0, self.game_state.energy - 5)
                                    self.game_state.add_exp(3)
                                    self.show_temporary_message("🥚 Fed the chickens & collected an egg! +1 Egg, -1 Corn, -5 energy", (100,200,100))
                                else:
                                    self.show_temporary_message("Not enough energy! Need 5 energy!", (200,100,100))
                            else:
                                self.show_temporary_message("✗ Need 1 corn to feed the chickens!", (200,100,100))
                        else:
                            self.show_temporary_message("Stand inside the pasture or chicken pen to feed animals!", (200,200,100))
                    elif event.key == pygame.K_r:
                        gate = self.world.toggle_nearby_gate(self.player.tx, self.player.ty)
                        if gate:
                            state = "opened" if gate.is_open else "closed"
                            self.show_temporary_message(f"Gate {state}!", (100,200,100))
                        else:
                            self.show_temporary_message("Stand near a gate to open/close it!", (200,200,100))
                    elif event.key == pygame.K_p:
                        sel = self.inventory.get_selected_item()
                        if sel and sel.endswith("_seed"):
                            if self.inventory.items.get(sel, 0) <= 0:
                                self.show_temporary_message("No seeds left for this crop!", (200, 100, 100))
                            elif self.game_state.energy >= 8:
                                crop_name = sel.replace("_seed", "").capitalize()
                                title = f"Planting {crop_name} - Quick Quiz!"
                                q = self.question_manager.get_question(
                                    f"plant_{sel}",
                                    title,
                                    fallback_categories=("plant_general", "general"),
                                )
                                if q:
                                    self.pending_action = {"type": "plant", "seed": sel,
                                                           "x": self.player.tx, "y": self.player.ty}
                                    self.current_question = QuestionPopup(q, self.on_question_complete)
                                else:
                                    self.show_temporary_message("No planting questions available.", (200, 100, 100))
                            else:
                                self.show_temporary_message("Not enough energy! Need 8 energy!", (200,100,100))
                    elif event.key == pygame.K_h:
                        crop = self.world.crops.get((self.player.tx, self.player.ty))
                        if crop and crop["growth"] >= 1.0:
                            if self.game_state.energy >= 5:
                                crop_type = crop["type"]
                                title = f"Harvesting {crop_type.capitalize()} - Quick Quiz!"
                                q = self.question_manager.get_question(
                                    f"harvest_{crop_type}",
                                    title,
                                    fallback_categories=("harvest_general", "general")
                                )
                                if q:
                                    self.pending_action = {"type": "harvest", "crop": crop_type,
                                                           "x": self.player.tx, "y": self.player.ty}
                                    self.current_question = QuestionPopup(q, self.on_question_complete)
                                else:
                                    self.show_temporary_message("No harvesting questions available.", (200, 100, 100))
                            else:
                                self.show_temporary_message("Not enough energy! Need 5 energy!", (200, 100, 100))
                        else:
                            self.show_temporary_message("No mature crop to harvest here.", (200, 200, 200))
                        ...
                        if sel and sel.endswith("_seed"):
                            if self.inventory.items.get(sel, 0) <= 0:
                                ...
                                if q:
                                    ...
                                    self.current_question = QuestionPopup(
                                        q,
                                        self.on_question_complete,
                                    )
                                else:
                                    self.show_temporary_message(
                                        "No planting questions available.",
                                        (200, 100, 100),
                                    )

                            else:
                                self.show_temporary_message(
                                    "Not enough energy! Need 8 energy!",
                                    (200, 100, 100),
                                )
                    elif event.key == pygame.K_h:
                        if self.game_state.energy >= 5:
    ...
    def on_question_complete(self, success):
        if self.pending_action:
            ...
            x = action.get("x", self.player.tx)
            y = action.get("y", self.player.ty)
        ...
        if success:
            if action["type"] == "plant":
                ...
                if (
                    self.inventory.items.get(seed, 0) > 0
                    and self.world.plant_crop(
                        x,
                        y,
                        seed,
                        self.game_state,
                    ...
                    crop_name = seed.replace("_seed", "").capitalize()

                    self.show_temporary_message(
                        f"{crop_name} planted! -8 energy",
                        (100, 200, 100),
                    )
                else:
                    self.show_temporary_message(
                        "Could not plant this here.",
                        (200, 100, 100),
                    )

            elif action["type"] == "harvest":
                harvested = self.world.harvest_crop(
                    x,
                    y,
                    self.inventory,
                    self.game_state,
                ...
                if harvested:
                    ...
                    self.show_temporary_message(
                        f"{harvested.capitalize()} harvested! -5 energy",
                        (100, 200, 100),
                    )
                else:
                    self.show_temporary_message(
    ...
    def update(self):
        if self.game_mode == "highway" and self.highway_scene:
            status = self.highway_scene.update(self.dt)
            if status == "arrived":
                self.game_mode = "farm"
                self.highway_scene = None
        ...
        if not self.tutorial.active and not self.quiz.active and not self.current_question:
            ...
            self.water_tap.update(self.dt, self.player)
    ...
    def draw(self):
        if self.game_mode == "highway" and self.highway_scene:
            self.highway_scene.draw(self.screen)
            pygame.display.flip()
            return

        self.world.draw(self.screen, self.camera_x, self.camera_y, self.game_state, self.game_state.zoom)
        self.water_tap.draw(self.screen, self.camera_x, self.camera_y, self.game_state.zoom, self.player)
        for a in self.animals:
        ...
        self.water_tap.draw_carried_bucket(self.screen, self.camera_x, self.camera_y, self.game_state.zoom, self.player)
